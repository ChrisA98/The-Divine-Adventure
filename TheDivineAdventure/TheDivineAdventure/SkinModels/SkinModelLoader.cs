...
namespace TheDivineAdventure.SkinModels
{
    ...
    class SkinModelLoader
    {
        ...
        #endregion
        public static int LoadingLevelPreset = 0;
        ...
        private void CreateMeshesAndBones(SkinModel model, Assimp.Scene scene, int meshIndex)
        {
            ...
            for (int mi = 0; mi < scene.Meshes.Count; mi++)
            {
                ...
                flatBone.offset_mtx = Matrix.CreateTranslation(new Vector3(10, 10, 10));
                ...
                for (int abi = 0; abi < assimpMeshBones.Count; abi++)
                {      // loop thru bones
        ...
        private void SetupMaterialsAndTextures(SkinModel model, Assimp.Scene scene)
        {
            ...
            {                // loop thru scene meshes
                ...
                for (int t = 0; t < assimpMaterialTextures.Length; t++)
                {              // loop thru textures
        ...
        private void CreateTreeTransforms(SkinModel model, SkinModel.ModelNode modelNode, Node curAssimpNode, int tabLevel)
        {
            ...
            if (curAssimpNode.HasMeshes)
            {
                ...
                foreach (int meshIndex in curAssimpNode.MeshIndices)
